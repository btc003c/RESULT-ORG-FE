...
export default function SchemaEditorPage() {
  ...
  useEffect(() => {
    if (!datasetId) return;
    api.datasets.getSchema(datasetId).then(schemaRes => {
      if (schemaRes && schemaRes.schemaDefinition) {
        setName(schemaRes.schemaDefinition.title || '');
        setDesc(schemaRes.schemaDefinition.description || '');
        const props = schemaRes.schemaDefinition.properties || {};
        const reqs = schemaRes.schemaDefinition.required || [];
        const loadedFields = Object.keys(props).map((key, i) => {
          const p = props[key];
          return {
            id: `f-${i}`,
            name: key,
            type: p.type === 'number' ? 'Number' : p.type === 'boolean' ? 'Boolean' : 'Text',
            required: reqs.includes(key),
            searchable: p.searchable || false,
            filterable: p.filterable || false,
            sortable: p.sortable || false,
          };
        });
        setFields(loadedFields.length > 0 ? loadedFields : [{ id: 'f-1', name: 'exampleField', type: 'Text', required: false, searchable: false, filterable: false, sortable: false }]);
      }
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      await api.datasets.updateSchema(datasetId, {
  ...
  return (
    <div className="max-w-[1200px] mx-auto font-sans pb-16 space-y-8">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-extrabold tracking-tight text-zinc-950">Schema Editor</h1>
          <p className="text-zinc-500 text-xs mt-0.5">Modify data schemas for your published database.</p>
        </div>
      </div>
